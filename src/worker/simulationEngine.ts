...
import {
  analyzeTransaction as analyzeFraud,
  createFraudAlert,
...
async function generateAccountTransactions(
  ...
  if (isWeekend(now)) {
    activityMultiplier *=
      config.weekendMultiplier * persona.weekendBehavior.multiplier;
  }

  // Base transaction probability per hour
  const hourlyFreq =
    (persona.transactionFrequency.min + persona.transactionFrequency.max) /
    2 /
    168; // Per week to per hour
  ...
  if (
    isRentDay(now) &&
    persona.categoryWeights.some((w) => w.category === "rent")
  ) {
    const rentWeight = persona.categoryWeights.find(
      (w) => w.category === "rent",
    );
    if (rentWeight && rng.next() < 0.9) {
      // 90% chance to pay rent
  ...
  for (let i = 0; i < numTransactions; i++) {
    ...
    const category = rng.pickWeighted(persona.categoryWeights)
...
async function analyzeTransactionForFraudAndRisk(
  ...
  try {
    ...
    if (fraudResult.isFraudulent || fraudResult.alerts.length > 0) {
      ...
      const hasCritical = fraudResult.alerts.some(
      ...
      const reason =
        fraudResult.alerts[0]?.reason ||
        "Automated fraud detection flagged this transaction";
    ...
    if (transaction.amount >= 100000) {
      // $1000+
      const amlResult = await analyzeTransactionAML(
        account.userId,
        transaction.amount,
      );
...
export async function runSimulationCycle(): Promise<{
  ...
  try {
    ...
    await updateSimulationState({
      lastRunAt: new Date(),
      transactionsToday: isNewDay
        ? totalTransactions
        : state.transactionsToday + totalTransactions,
      isRunning: false,
    });
