...
describe('Phase 3 Integration Tests', () => {
  ...
  describe('Chaos Mode Control', () => {
    ...
    it('POST /sandbox/mode/reset returns to normal', async () => {
      ...
      // Reset
      const { status, data } = await request('/sandbox/mode/reset', { method: 'POST' });
  ...
  describe('Account Freeze/Unfreeze', () => {
    ...
    it('PATCH /sandbox/account/:id/freeze freezes account', async () => {
      ...
      const { status, data } = await request(`/sandbox/account/${testAccountId}/freeze`, {
        method: 'PATCH',
      });
    ...
    it('PATCH /sandbox/account/:id/unfreeze unfreezes account', async () => {
      ...
      // Unfreeze
      const { status, data } = await request(`/sandbox/account/${testAccountId}/unfreeze`, {
        method: 'PATCH',
      });
    ...
    it('returns 404 for non-existent account', async () => {
      const { status } = await request('/sandbox/account/non-existent-id/freeze', {
        method: 'PATCH',
      });
  ...
  describe('Account Balance Control', () => {
    it('PATCH /sandbox/account/:id/set-balance updates balance', async () => {
      ...
      const { status, data } = await request(`/sandbox/account/${testAccountId}/set-balance`, {
        method: 'PATCH',
    ...
    it('PATCH /sandbox/account/:id/set-balance rejects invalid balance', async () => {
      ...
      const { status } = await request(`/sandbox/account/${testAccountId}/set-balance`, {
        method: 'PATCH',
  ...
  describe('Account Settings', () => {
    it('PATCH /sandbox/account/:id/settings updates overdraft and daily limit', async () => {
      ...
      const { status, data } = await request(`/sandbox/account/${testAccountId}/settings`, {
        method: 'PATCH',
  ...
  describe('Transaction Lifecycle', () => {
    it('POST /sandbox/transactions/process-pending processes pending transactions', async () => {
      const { status, data } = await request('/sandbox/transactions/process-pending', {
        method: 'POST',
      });
