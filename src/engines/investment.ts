...
export type PortfolioType = 'conservative' | 'balanced' | 'aggressive' | 'crypto';

// Default market assets for simulation
const DEFAULT_ASSETS = [
  // Stocks
  { symbol: 'AAPL', name: 'Apple Inc.', type: 'stock', price: 18500, volatility: 0.02 },
  { symbol: 'GOOGL', name: 'Alphabet Inc.', type: 'stock', price: 14200, volatility: 0.025 },
  { symbol: 'MSFT', name: 'Microsoft Corp.', type: 'stock', price: 37500, volatility: 0.018 },
  { symbol: 'AMZN', name: 'Amazon.com Inc.', type: 'stock', price: 17800, volatility: 0.028 },
  { symbol: 'TSLA', name: 'Tesla Inc.', type: 'stock', price: 24200, volatility: 0.045 },
  { symbol: 'NVDA', name: 'NVIDIA Corp.', type: 'stock', price: 48000, volatility: 0.04 },
  
  // ETFs
  { symbol: 'SPY', name: 'S&P 500 ETF', type: 'etf', price: 47500, volatility: 0.012 },
  { symbol: 'QQQ', name: 'Nasdaq-100 ETF', type: 'etf', price: 40000, volatility: 0.018 },
  { symbol: 'VTI', name: 'Total Stock Market ETF', type: 'etf', price: 24000, volatility: 0.011 },
  { symbol: 'BND', name: 'Total Bond Market ETF', type: 'etf', price: 7200, volatility: 0.005 },
  
  // Crypto
  { symbol: 'BTC', name: 'Bitcoin', type: 'crypto', price: 4250000, volatility: 0.06 },
  { symbol: 'ETH', name: 'Ethereum', type: 'crypto', price: 230000, volatility: 0.07 },
  { symbol: 'SOL', name: 'Solana', type: 'crypto', price: 15000, volatility: 0.08 },
  { symbol: 'DOGE', name: 'Dogecoin', type: 'crypto', price: 15, volatility: 0.12 },
  
  // Bonds/Savings
  { symbol: 'TBILL', name: 'Treasury Bills', type: 'bond', price: 10000, volatility: 0.001 },
  { symbol: 'HYS', name: 'High Yield Savings', type: 'savings', price: 10000, volatility: 0.0001 },
];

// Portfolio allocations by type
const PORTFOLIO_ALLOCATIONS: Record<PortfolioType, Array<{ symbol: string; weight: number }>> = {
  conservative: [
    { symbol: 'BND', weight: 0.40 },
    { symbol: 'SPY', weight: 0.30 },
    { symbol: 'VTI', weight: 0.20 },
    { symbol: 'HYS', weight: 0.10 },
  ],
  balanced: [
...
export async function createPortfolio(
  ...
  const portfolio = await prisma.portfolio.create({
    data: {
      userId,
      name: name || `${portfolioType.charAt(0).toUpperCase() + portfolioType.slice(1)} Portfolio`,
      type: portfolioType,
      totalInvested: initialInvestment,
      riskTolerance: portfolioType === 'crypto' || portfolioType === 'aggressive' ? 'high' :
                      portfolioType === 'conservative' ? 'low' : 'medium',
    },
  });
...
export async function updateMarketPrices(
  ...
  for (const asset of assets) {
    ...
    const baseChange = (rng.next() * 2 - 1) * asset.volatility * volatilityMultiplier;
...
export async function triggerMarketCrash(
  ...
  for (const asset of assets) {
    ...
    const newPrice = Math.max(1, Math.floor(asset.price * (1 - finalCrashAmount)));
...
export async function getMarketOverview(): Promise<{
  ...
  const marketStatus = avgChange > 0.01 ? 'bull' : avgChange < -0.01 ? 'bear' : 'neutral';
...
export async function getInvestmentSummary(): Promise<{
  ...
  const avgReturn = totalInvested > 0 ? (totalGainLoss / totalInvested) * 100 : 0;
