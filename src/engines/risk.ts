 ...
 */
export async function calculateUserRiskScore(userId: string): Promise<RiskAssessment> {
  ...
  const weights = {
    ...
    overdraftFrequency: 0.20,
    repaymentDiscipline: 0.20,
...
async function calculateRiskFactors(user: any): Promise<RiskFactors> {
  ...
  // Overdraft Frequency
  const overdraftCount = user.accounts.reduce((sum: number, a: any) => 
    sum + (a.overdraftUsed > 0 ? 1 : 0) + countNegativeBalanceDays(a.transactions), 0);
  const overdraftFrequency = Math.min(100, overdraftCount * 10);
  
  // Repayment Discipline
  const paymentsMade = allLoans.reduce((sum: number, l: any) => sum + l.paymentsMade, 0);
  const paymentsMissed = allLoans.reduce((sum: number, l: any) => sum + l.paymentsMissed, 0);
  const totalPayments = paymentsMade + paymentsMissed;
  const repaymentDiscipline = totalPayments > 0 
    ? Math.round((paymentsMade / totalPayments) * 100) 
    : 80; // Default for users with no loan history
  ...
  const oldestAccount = user.accounts.reduce((oldest: Date, a: any) => 
    a.createdAt < oldest ? a.createdAt : oldest, new Date());
  const accountAgeDays = Math.floor((Date.now() - oldestAccount.getTime()) / (24 * 60 * 60 * 1000));
  const accountAge = Math.min(100, accountAgeDays / 3.65); // 365 days = 100
  
  // Transaction Diversity (variety in categories)
  const categories = new Set(allTransactions.map((t: any) => t.category).filter(Boolean));
...
function generateRecommendations(factors: RiskFactors, level: RiskLevel): string[] {
  ...
  if (factors.spendingStability < 40) {
    recommendations.push('Consider setting up a budget to stabilize spending patterns');
  }
  if (factors.incomeVolatility > 60) {
    recommendations.push('Build an emergency fund to handle income fluctuations');
  }
  ...
  if (factors.repaymentDiscipline < 70) {
    recommendations.push('Set up automatic loan payments to improve repayment history');
  }
  ...
  if (factors.accountAge < 30) {
    recommendations.push('Build account history by maintaining consistent activity');
  }
  
  if (level === 'critical') {
    recommendations.unshift('⚠️ Immediate attention required: High risk profile detected');
  }
...
export async function analyzeTransactionRisk(
  ...
  const avgSpending = account.transactions
    .filter(t => t.type === 'debit')
    .reduce((sum, t) => sum + t.amount, 0) / Math.max(1, account.transactions.length);
  
  if (type === 'debit' && amount > avgSpending * 3 && amount > 50000) { // $500+
  ...
  const commonLocations = account.transactions
    .map(t => t.location)
    .filter(Boolean);
  if (location && commonLocations.length > 5 && !commonLocations.includes(location)) {
  ...
  if (type === 'debit' && category === 'withdrawal' && amount > 100000) { // $1000+
...
function countNegativeBalanceDays(transactions: any[]): number {
  ...
  return transactions.filter(t => t.type === 'debit' && t.amount > 100000).length;
}
